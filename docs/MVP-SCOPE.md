...
- ~~The Windows Task Scheduler XML generator. Three lines of README instead.~~
  **Reversed, 4 Sep 2026.** The cut rested on two objections: it "isn't a real
  install", and it was "generated by a code path you cannot test on CI you don't
  have". Both are now answerable. `service install` registers the task for real
  with `schtasks /Create /XML`, and the Windows CI job installs it, queries it
  and removes it on every push, so Task Scheduler validates the definition rather
  than a reviewer.
