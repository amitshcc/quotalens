...
`i-settings` is **a gear, and the one filled icon in this set** — a deliberate,
narrow exception: still 16×16, still `currentColor`, still in the sprite, fill
rather than stroke. An earlier version was sliders, on the argument that a
gear's teeth are a different drawing language at 16px. That objection was about
legibility, and the answer is to draw the gear well rather than draw something
else.

Both constructions were rendered beside `i-sun` at 16px in light theme, which is
the hardest case: stroked — a hub, a body circle and six radial ticks — merges
into texture at that size, while the silhouette keeps its teeth as distinct
bumps. **The body is also what stops it reading as `i-sun`**, which is a circle
with eight rays sitting in the same toolbar; what makes a gear a gear is that
the teeth sit on a body, not that they radiate from a centre.

The path is generated by `design/render_gear.py` from a radius and an angle
step. Teeth nudged by hand cannot be redrawn at another size later.
